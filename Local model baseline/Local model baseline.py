...
def mol_features(smiles):
    """[RDKit_2D, Morgan_ECFP4 (2048, 0/1)] 定长向量"""
...
def build_Xy(df: pd.DataFrame):
    ...
    col_names = [f"RDKit_{n}" for n in RDKIT_NAMES] + [f"Morgan_{i}" for i in range(MORGAN_BITS)]
    X.columns = col_names[:len(X.columns)]
    X = X.dropna(axis=1, how="all").loc[:, lambda d: d.var() > 0].reset_index(drop=True)
    return X, df[RT_COL].to_numpy()


# =================== 10 折 OOF CV ===================
def evaluate_dataset(df: pd.DataFrame):
    """返回 (metrics, y_true, oof, top_feats)"""
    if len(df) < N_SPLITS:
    ...
    metrics = {"n": len(df), "Q2": r2_score(y, oof),
...
def main():
    ...
    top20 = select_top20(count_datasets(REPORT_DIR))
    print("前 20 个数据集 (RT 记录数降序):")
    for _, r in top20.iterrows():
        print(f"  {r['dataset_id']}\t{r['source']}\t{r['n_records']}")

    results, all_oof, all_top_features = [], [], []
    for _, r in top20.iterrows():
        ds = str(r["dataset_id"])
        ...
        if path is None:
            continue
        metrics, y_true, oof, top_feats = evaluate_dataset(read_tsv(path))
        metrics.update(dataset_id=ds, source=source)
        results.append(metrics)
        all_top_features.append(dict(top_feats))
        all_oof.append(pd.DataFrame({"dataset_id": ds, "y_true": y_true, "y_pred": oof}))
        print(f"  {ds}: n={metrics['n']}, Q2={metrics['Q2']:.4f}, "
              f"MAE={metrics['MAE']:.4f}, RMSE={metrics['RMSE']:.4f}")

    # ================= 保存 3 个 CSV 到 output/ =================
    results_df = pd.DataFrame(results)
    oof_df = pd.concat(all_oof, ignore_index=True)
    results_df.to_csv(BASE / "experiment_A_results.csv", index=False)
    oof_df.to_csv(BASE / "experiment_A_oof.csv", index=False)

    feat_counter = Counter()
    for d in all_top_features:
        for name, val in d.items():
            feat_counter[name] += val
    pd.DataFrame(feat_counter.items(), columns=["feature", "gain"]) \
        .sort_values("gain", ascending=False) \
        .to_csv(BASE / "experiment_A_feature_importance.csv", index=False)

    print(f"\n[已保存] -> {BASE.resolve()}")
    print(f"  experiment_A_results.csv ({len(results_df)} datasets)")
    print(f"  experiment_A_oof.csv ({len(oof_df)} rows)")
    print(f"  experiment_A_feature_importance.csv ({len(feat_counter)} features)")
